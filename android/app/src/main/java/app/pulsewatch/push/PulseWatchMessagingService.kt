package app.pulsewatch.push

import android.util.Log
import app.pulsewatch.PulseWatchApp
import app.pulsewatch.core.api.PushPlatform
import app.pulsewatch.core.api.PushTokenInput
 ...
 */
class PulseWatchMessagingService : FirebaseMessagingService() {

    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    override fun onNewToken(token: String) {
        val app = applicationContext as? PulseWatchApp ?: return
        scope.launch {
            runCatching {
    ...
    private companion object { const val TAG = "PulseWatchFcm" }
}
